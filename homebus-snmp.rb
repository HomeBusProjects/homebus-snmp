...
class SNMPHomeBusApp < HomeBusApp
  ...
  def setup!
    ...
    response.each_varbind do |vb|
      ...
      @sysName = vb.value.to_s if  vb.name.to_s == 'sysName.0'
      @sysDescr = vb.value.to_s if  vb.name.to_s == 'sysDescr.0'
    end
    ...
    response = manager.get(Range.new(0, interface_count).map { |i| "ifName.#{i}" })
    response.each_varbind do |vb|
      puts "#{vb.name.to_s}  #{vb.value.to_s}  #{vb.value.asn1_type}"
    end
  end
...
homebus_request = {
  ...
  friendly_name: 'HomeBus Router monitor',
  friendly_location: '10.0.1.1',
  pin: '',
  devices: devices
}
